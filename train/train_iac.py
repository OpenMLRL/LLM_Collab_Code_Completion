"""IAC training entrypoint for ClassEval module completion with collaboration."""
...
from comlrl.trainers.iac import IACConfig, IACTrainer  # type: ignore
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Train IAC for ClassEval module completion")
    parser.add_argument(
        "--config",
        type=str,
        default=os.path.join(REPO_ROOT, "configs", "iac_classeval_config.yaml"),
    ...
    output_dir_cfg = iac_cfg.get("output_dir") or output_cfg.get(
        "base_dir", os.path.join(os.getcwd(), "output")
    )
    output_dir = str(output_dir_cfg)
    iac_cfg["output_dir"] = output_dir
    try:
    ...
    if num_agents == 1:
        ...
        def reward_func(prompts: List[str], agent1_completions: List[str], agent2_completions: List[str]) -> List[float]:
            item = _lookup_item(prompts)
    ...
    try:
        ...
        pass
    try:
        num_turns_val = int(getattr(iac_args, "num_turns", 1) or 1)
    except Exception:
        num_turns_val = 1
    try:
        eval_samples = int(getattr(iac_args, "eval_num_samples", 0) or 0)
    except Exception:
        eval_samples = 0
    try:
