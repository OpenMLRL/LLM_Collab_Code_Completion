...
- lv2 (total-picks control): let S = Σ_i |A_i| be the total number of functions generated by all agents.
  Piecewise quadratic (assuming V>0):
    - Left branch (S ≤ V): concave-down parabola through (0,-1), (V,2), (2V,-1)
        lv2(S) = 2 - 3 * ((S - V)^2) / V^2
    - Right branch (V < S ≤ 2V+2): parabola through (V,2), (2V+1,-1), (2V+2,-2)
...
def get_reward_function(strategy, num_agents: int) -> Callable[..., List[float]]:
    ...
    - lv2 = total-picks control:
      Let S = Σ_i |A_i| be the total number of functions generated by all agents.
        ...
        * If V < S <= 2V+2: lv2(S) is the quadratic through (V,2), (2V+1,-1), (2V+2,-2)
    ...
    def reward_wrapper(*agent_completions, batch_items=None, prompts=None):
        ...
        for i in range(batch_size):
            ...
            # - Right branch (updated): parabola passing (V, 2), (2V+1, -1), (2V+2, -2)
            if V > 0:
                if S_total <= V:
                    dv = float(S_total) - float(V)
                    lv2 = 2.0 - 3.0 * (dv * dv) / (float(V) * float(V))
                else:
                    # Right branch: p(S) = 2 + A*(S - V) + B*(S - V)^2
                    # A = (V^2 - 4V - 8) / (V^2 + 3V + 2)
                    # B = (2 - V) / ((V + 1) * (V + 2))
                    dv = float(S_total) - float(V)
                    denom = float(V * V + 3 * V + 2)
                    A = float(V * V - 4 * V - 8) / denom
                    B = float(2 - V) / float((V + 1) * (V + 2))
                    lv2 = 2.0 + A * dv + B * dv * dv
            else:
                lv2 = -2.0 * INF
            # numerical guard
            # if lv2 > 2.0:
